...
class SpeechModel(SerializableModule):
    def __init__(self, config):
        ...
        if "conv2_size" in config:
            ...
            n_featmap2 = config["n_feature_maps2"]
            self.conv2 = nn.Conv2d(n_featmaps1, n_featmap2, conv2_size, stride=conv2_stride)
            if tf_variant:
...
# class simple_CNN(SerializableModule):
#     def __init__(self, config):
#         super().__init__()
#         self.conv1 = nn.Conv2d(in_channels=1, 
#                                out_channels=config["n_feature_maps1"],
#                                )
...
class SpeechDataset(data.Dataset):
    ...
    def __init__(self, data, set_type, config):
        ...
        self.bg_noise_audio = [librosa.core.load(file, sr=config["sample_rate"])[0] for file in config["bg_noise_files"]]
        ...
        n_unknown = len(list(filter(lambda x: x == 1, self.audio_labels)))
        self.n_silence = int(self.silence_prob * (len(self.audio_labels) - n_unknown))
    ...
    def default_config():
        ...
        config["length_second"] = 1
        config["input_length"] = config["sample_rate"] * config["length_second"]
        config["n_mels"] = 40
        ...
        config["dev_pct"] = 10
        config["test_pct"] = 10
        config["wanted_words"] = ["yes", "no"]
        config["data_folder"] = "./data/speech_commands_v0.02"
    ...
    def _timeshift_audio(self, data):
        shift = (self.input_length * self.timeshift_ms) // 1000
